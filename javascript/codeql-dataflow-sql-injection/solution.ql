 ...
 */
 ...
 class ReadFileSyncCall extends API::CallNode {
  ReadFileSyncCall() {
      this =  API::moduleImport("fs").getMember("readFileSync").getACall()
    }
    ...
    class SqliteCall extends DataFlow::SourceNode {
      SqliteCall(){
        this = API::moduleImport("sqlite3").getMember("verbose").getACall().getAConstructorInvocation("Database")
      }
    }
 ...
 class DatabaseNew extends DataFlow::InvokeNode {
  DatabaseNew(){
    this.getCalleeName() = "Database"
  }
 }

 class ExecCall extends MethodCallExpr{
  ExecCall(){
    this.getMethodName() = "exec"
  }
 }
 ...
 class UltimateFlowCfg extends TaintTracking::Configuration {
   ...
   override predicate isSource(DataFlow::Node nd) { 
   exists(ReadFileSyncCall r | nd = r)
   }
